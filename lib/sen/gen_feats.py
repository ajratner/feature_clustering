...
# All the rest should be considered more or less private, except the #
...
def get_generic_features_relation(sentence, span1, span2):
    ...
    for seq_feat in _get_seq_features(
            sentence, betw_span, prefix="SEQ_" + inverted):
        yield seq_feat
    # Window feature (left and right, up to size 3, combined)
    for window_feat in _get_window_features(
            sentence, covering_span, isolated=False, prefix="SEQ_" + inverted +
            "WIN_"):
        yield window_feat
    # Ngrams of up to size 3 between the mentions
    for ngram_feat in _get_ngram_features(
            sentence, betw_span, prefix="SEQ_" + inverted + "NGRAM_"):
        yield ngram_feat
    # Indicator features of whether the mentions are in dictionaries
    found1 = False
    for feat1 in _get_dictionary_indicator_features(
            sentence, span1, prefix="IND_" + inverted + "DICT_"):
        found1 = True
        found2 = False
    ...
    for (i, j) in _get_substring_indices(len(sentence), MAX_KW_LENGTH):
        ...
        for dict_id in dictionaries:
            if " ".join(map(lambda x: str(x.lemma), sentence[i:j])) in \
                    dictionaries[dict_id]:
                is_in_dictionary = True
                yield "IND_" + inverted + "KW_" + FEAT_SEP + dict_id
                break
        if is_in_dictionary:
            kw_span = Span(begin_word_id=i, length=j-i)
            ...
            labels1 = []
            for edge in path1:
                lemmas1.append(WORD_PRE + str(edge.word2.lemma))
            ...
            labels2 = []
            for edge in path2:
                lemmas2.append(WORD_PRE + str(edge.word2.lemma))
            ...
            both2 = both2[:-1]
            yield "DEP_" + inverted + "KW_" + FEAT_SEP + \
                WORD_SEP.join(both1) + FEAT_SEP + WORD_SEP.join(both2)
            yield "DEP_" + inverted + "KW_L_" + FEAT_SEP + \
                WORD_SEP.join(labels1) + FEAT_SEP + WORD_SEP.join(labels2)
            ...
            for j in range(1, len(both2), 2):
                for dict_id in dictionaries:
                    if both2[j] in dictionaries[dict_id]:
                        ...
                        break  # Picking up the first dictionary we find
            yield "DEP_" + inverted + "KW_D_" + FEAT_SEP + \
                WORD_SEP.join(both1) + FEAT_SEP + WORD_SEP.join(both2)
    ...
    capital_feat = "IND_" + inverted + "STARTS_WITH_CAPITAL_" + FEAT_SEP + \
        str(first_capital) + FEAT_SEP + str(second_capital)
    yield capital_feat
    ...
    length_feat = "LEN_" + inverted + "LENGTHS_" + FEAT_SEP + first_length + \
        FEAT_SEP + second_length
    yield length_feat
...
def _get_ngram_features(sentence, span, window=3, prefix="SEQ_NGRAM_"):
    ...
    for i in range(span.begin_word_id, span.begin_word_id + span.length):
        for j in range(1, window + 1):
            if i+j <= span.begin_word_id + span.length:
                yield prefix + str(j) + "_" + FEAT_SEP + WORD_SEP.join(
                    map(lambda x: WORD_PRE + str(x.lemma), sentence[i:i+j]))
...
def _get_min_dep_path_features(sentence, span1, span2, prefix="DEP_BETW_"):
    ...
    if min_path:
        ...
        for edge in min_path:
            min_path_lemmas.append(WORD_PRE + str(edge.word2.lemma))
        ...
        both = both[:-1]
        yield prefix + FEAT_SEP + WORD_SEP.join(both)
        yield prefix + "L_" + FEAT_SEP + WORD_SEP.join(min_path_labels)
        for j in range(1, len(both), 2):
            for dict_id in dictionaries:
                if both[j] in dictionaries[dict_id]:
                    both[j] = "DICT_" + str(dict_id)
                    break  # Picking up the first dictionary we find
        yield prefix + "D_" + FEAT_SEP + WORD_SEP.join(both)


def _get_seq_features(sentence, span, prefix="SEQ_"):
    ...
    Args:
        ...
        span: the Span
    """
    word_seq_feat = prefix + "WORD_" + FEAT_SEP + \
        WORD_SEP.join(materialize_span(sentence, span, lambda x: str(x.word)))
    yield word_seq_feat
    lemma_seq_feat = prefix + "LEMMA_" + FEAT_SEP + \
        WORD_SEP.join(
            materialize_span(sentence, span, lambda x: WORD_PRE + str(x.lemma)))
    yield lemma_seq_feat
    ner_seq_feat = prefix + "NER_" + FEAT_SEP + WORD_SEP.join(materialize_span(
        sentence, span, lambda x: str(x.ner)))
    yield ner_seq_feat
    pos_seq_feat = prefix + "POS_" + FEAT_SEP + WORD_SEP.join(materialize_span(
    ...
    try:
        for i in range(1, window + 1):
            ...
            try:
                ...
                pass
            left_lemmas.append(WORD_PRE + lemma)
    ...
    try:
        for i in range(1, window + 1):
            ...
            try:
                ...
                pass
            right_lemmas.append(WORD_PRE + lemma)
            right_ners.append(str(sentence[span_end_idx + i].ner))
    except IndexError:
        pass
    if isolated:
        for i in range(len(left_lemmas)):
            yield prefix + "LEFT_" + str(i+1) + "_" + FEAT_SEP + \
                WORD_SEP.join(left_lemmas[-i-1:])
            yield prefix + "LEFT_NER_" + str(i+1) + "_" + FEAT_SEP + \
                WORD_SEP.join(left_ners[-i-1:])
        for i in range(len(right_lemmas)):
            yield prefix + "RIGHT_" + str(i+1) + "_" + FEAT_SEP + \
                WORD_SEP.join(right_lemmas[:i+1])
            yield prefix + "RIGHT_NER_" + str(i+1) + "_" + FEAT_SEP + \
                WORD_SEP.join(right_ners[:i+1])
    if combinations:
        for i in range(len(left_lemmas)):
            curr_left_lemmas = WORD_SEP.join(left_lemmas[-i-1:])
            try:
                curr_left_ners = WORD_SEP.join(left_ners[-i-1:])
            except TypeError:
                new_ners = []
                ...
                curr_left_ners = WORD_SEP.join(new_ners)
            for j in range(len(right_lemmas)):
                curr_right_lemmas = WORD_SEP.join(right_lemmas[:j+1])
                try:
                    curr_right_ners = WORD_SEP.join(right_ners[:j+1])
                except TypeError:
                    new_ners = []
                ...
                yield prefix + "LEMMA_L_" + str(i+1) + "_R_" + str(j+1) + \
                    "_" + FEAT_SEP + curr_left_lemmas + FEAT_SEP + \
                    curr_right_lemmas
                yield prefix + "NER_L_" + str(i+1) + "_R_" + str(j+1) + \
                    "_" + FEAT_SEP + curr_left_ners + FEAT_SEP + \
                    curr_right_ners


def _get_dictionary_indicator_features(
        sentence, span, window=3, prefix="IND_DICT_"):
...
the dictionaries
    ...
    for dict_id in in_dictionaries:
        yield prefix + FEAT_SEP + str(dict_id)
